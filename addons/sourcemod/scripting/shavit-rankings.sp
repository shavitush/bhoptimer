...
void UpdatePlayerPoints(int client)
{
	...
	if(GetClientAuthId(client, AuthId_Steam3, sAuthID, 32))
	{
		...
		FormatEx(sQuery, 512, "UPDATE %susers u JOIN (SELECT SUM((points * (@f := 0.975 * @f) / 0.975)) total FROM %splayertimes t " ...
			"CROSS JOIN (SELECT @f := 1.0) params WHERE points > 0.0 AND auth = '%s' ORDER BY points DESC) temp " ...
