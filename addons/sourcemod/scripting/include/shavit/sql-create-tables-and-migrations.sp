...
public void SQL_CreateTables(Database hSQL, const char[] prefix, int driver)
{
	...
	{
		FormatEx(sQuery, sizeof(sQuery),
			"CREATE TABLE IF NOT EXISTS %sstylepoints (auth INT NOT NULL, style SMALLINT NOT NULL, points REAL NOT NULL DEFAULT 0, PRIMARY KEY (auth, style));",
			gS_SQLPrefix);
	}
	else
	{
...
void ApplyMigration_AddStylePoints()
{
	...
	if (gI_Driver == Driver_mysql)
	{
		FormatEx(sQuery, sizeof(sQuery), "INSERT IGNORE INTO %sstylepoints (auth, style) SELECT DISTINCT auth, style FROM %splayertimes;", gS_SQLPrefix, gS_SQLPrefix);
	}
	else if (gI_Driver == Driver_pgsql)
	{
		FormatEx(sQuery, sizeof(sQuery), "INSERT INTO %sstylepoints (auth, style) SELECT DISTINCT auth, style FROM %splayertimes ON CONFLICT DO NOTHING;", gS_SQLPrefix, gS_SQLPrefix);
	}
	else // SQLite
	{
		FormatEx(sQuery, sizeof(sQuery), "INSERT OR IGNORE INTO %sstylepoints (auth, style) SELECT DISTINCT auth, style FROM %splayertimes;", gS_SQLPrefix, gS_SQLPrefix);
	}
