...
void LoadMapList()
{
	...
	switch (g_cvMapListType.IntValue)
	{
		case MapListZoned:
		{
			...
			FormatEx(buffer, sizeof(buffer), "SELECT UNIQUE `map` FROM `%smapzones` mz WHERE EXISTS (select 1 from `%smapzones` WHERE `map` = mz.`map` AND `type` = 0 AND `track` = 0) AND EXISTS (select 1 from `%smapzones` WHERE `map` = mz.`map` AND `type` = 1 AND `track` = 0) ORDER BY `map`", g_cSQLPrefix, g_cSQLPrefix, g_cSQLPrefix);
